// nvcc -I../../include -O2 -arch=compute_13 -code=sm_13 -use_fast_math -Xptxas=-v -maxrregcount=32 -cubin ssyrk.cu
#include "blas.h"
...
__device__ void saxpy(float a, const float * b, float * c) {
  ...
  c[ 8] += a * b[ 8]; c[ 9] += a * b[ 9]; c[10] += a * b[10]; c[11] += a * b[11];
  c[12] += a * b[12]; c[13] += a * b[13]; c[14] += a * b[14]; c[15] += a * b[15];
}
...
__global__ void ssyrk(int n, int k, float alpha,
...
#pragma unroll
  ...
  if (i < n) {
    ...
    if (beta == 0.0f) {
      ...
      if ((uplo == CBlasUpper && i <= j +  7) || (uplo == CBlasLower && i >= j +  7)) C[0] = alpha * c[ 7]; if ( 8 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j +  8) || (uplo == CBlasLower && i >= j +  8)) C[0] = alpha * c[ 8]; if ( 9 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j +  9) || (uplo == CBlasLower && i >= j +  9)) C[0] = alpha * c[ 9]; if (10 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 10) || (uplo == CBlasLower && i >= j + 10)) C[0] = alpha * c[10]; if (11 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 11) || (uplo == CBlasLower && i >= j + 11)) C[0] = alpha * c[11]; if (12 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 12) || (uplo == CBlasLower && i >= j + 12)) C[0] = alpha * c[12]; if (13 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 13) || (uplo == CBlasLower && i >= j + 13)) C[0] = alpha * c[13]; if (14 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 14) || (uplo == CBlasLower && i >= j + 14)) C[0] = alpha * c[14]; if (15 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 15) || (uplo == CBlasLower && i >= j + 15)) C[0] = alpha * c[15];
    }
    else {
      ...
      if ((uplo == CBlasUpper && i <= j +  7) || (uplo == CBlasLower && i >= j +  7)) C[0] = alpha * c[ 7] + beta * C[0]; if ( 8 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j +  8) || (uplo == CBlasLower && i >= j +  8)) C[0] = alpha * c[ 8] + beta * C[0]; if ( 9 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j +  9) || (uplo == CBlasLower && i >= j +  9)) C[0] = alpha * c[ 9] + beta * C[0]; if (10 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 10) || (uplo == CBlasLower && i >= j + 10)) C[0] = alpha * c[10] + beta * C[0]; if (11 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 11) || (uplo == CBlasLower && i >= j + 11)) C[0] = alpha * c[11] + beta * C[0]; if (12 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 12) || (uplo == CBlasLower && i >= j + 12)) C[0] = alpha * c[12] + beta * C[0]; if (13 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 13) || (uplo == CBlasLower && i >= j + 13)) C[0] = alpha * c[13] + beta * C[0]; if (14 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 14) || (uplo == CBlasLower && i >= j + 14)) C[0] = alpha * c[14] + beta * C[0]; if (15 >= n) return; C += ldc;
      if ((uplo == CBlasUpper && i <= j + 15) || (uplo == CBlasLower && i >= j + 15)) C[0] = alpha * c[15] + beta * C[0];
    }
  }
}
 ...
 */
template void ssyrk<CBlasUpper, CBlasNoTrans, 64, 16, 16, 16,  4>(int, int, float, const float *, int, float, float *, int);
template void ssyrk<CBlasLower, CBlasNoTrans, 64, 16, 16, 16,  4>(int, int, float, const float *, int, float, float *, int);
template void ssyrk<CBlasUpper, CBlasTrans,   32, 32,  8,  8,  8>(int, int, float, const float *, int, float, float *, int);
template void ssyrk<CBlasLower, CBlasTrans,   32, 32,  8,  8,  8>(int, int, float, const float *, int, float, float *, int);
