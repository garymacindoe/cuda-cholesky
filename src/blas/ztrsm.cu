...
// y(1:2) -= alpha * x(1:2)
__device__ void zaxpy(cuDoubleComplex alpha, const int * x_real_lo, const int * x_real_hi,
                      const int * x_imag_lo, const int * x_imag_hi, cuDoubleComplex * y) {
...
// y(1:n) -= alpha * x(1:n)
__device__ void zaxpy(int n, cuDoubleComplex alpha, const int * x_real_lo, const int * x_real_hi,
                      const int * x_imag_lo, const int * x_imag_hi, cuDoubleComplex * y) {
  y[0] = cuCsub(y[0], cuCmul(alpha, make_cuDoubleComplex(__hiloint2double(x_real_hi[0], x_real_lo[0]),
                                                         __hiloint2double(x_imag_hi[0], x_imag_lo[0]))));
  if (1 >= n) return;
    ...
    if ((uplo == CBlasUpper && transA == CBlasNoTrans) ||
      ...
      if (mm > 0) {
        ...
        for (int j = 0; j < nb; j += by) {
          ...
          b_imag_hi[threadIdx.x][j + threadIdx.y] = __double2loint(cuCimag(X[j * ldb]));
        }
        ...
        switch (mm - 1) {
          case 1:
            ...
            zaxpy(1, x[1], a_real_lo[1], a_real_hi[1], a_imag_lo[1], a_imag_hi[1], x);
          case 0:
            if (diag == CBlasNonUnit)
              ...
              zaxpy(0, x[0], a_real_lo[0], a_real_hi[0], a_imag_lo[0], a_imag_hi[0], x);
        }

        __syncthreads();

        // Write X out transposing it back via shared memory using b.
        b_imag[0][ti] = cuCreal(x[0]); b_real[1][ti] = cuCimag(x[1]);
        b_imag[1][ti] = cuCreal(x[1]); b_real[1][ti] = cuCimag(x[1]);
      ...
      while (i >= 0) {
        ...
        while (k > 0) {
          ...
          for (int l = 0; l < mb; l++)
            zaxpy(make_cuDoubleComplex(__hiloint2double(b_real_hi[l][ti], b_real_lo[l][ti]),
                  ...
                  a_real_lo[l], a_real_hi[l], a_imag_lo[l], a_imag_hi[l], x);
        ...
        // Write X out transposing it back via shared memory using b
        b_real_lo[0][ti] = __double2loint(cuCreal(x[0])); b_real_hi[0][ti] = __double2hiint(cuCreal(x[0]));
        b_imag_lo[0][ti] = __double2loint(cuCimag(x[0])); b_imag_hi[0][ti] = __double2loint(cuCimag(x[0]));
        b_real_lo[1][ti] = __double2loint(cuCreal(x[1])); b_real_hi[1][ti] = __double2hiint(cuCreal(x[1]));
        b_imag_lo[1][ti] = __double2loint(cuCimag(x[1])); b_imag_hi[1][ti] = __double2loint(cuCimag(x[1]));
      ...
      while (m > 0) {
        ...
        if (diag == CBlasNonUnit)
          x[0] = cuCdiv(x[0], make_cuDoubleComplex(__hiloint2double(a_real_hi[0][0], a_real_lo[0][0]),
                                                   __hiloint2double(a_imag_hi[0][0], a_imag_lo[0][0]));
        caxpy(1, x[0], &a_real_lo[0][1], &a_real_hi[0][1], &a_imag_lo[0][1], &a_imag_hi[0][1], &x[1]);
        if (diag == CBlasNonUnit)
          x[1] = cuCdiv(x[1], make_cuDoubleComplex(__hiloint2double(a_real_hi[1][1], a_real_lo[1][1]),
                                                   __hiloint2double(a_imag_hi[1][1], a_imag_lo[1][1]));
        caxpy(2, x[1], &a_real_lo[1][2], &a_real_hi[1][2], &a_imag_lo[1][2], &a_imag_hi[1][2], &x[2]);
        ...
        #pragma unroll
        for (int j = 0; j < nb; j += by)
          X[j * ldb] = make_cuDoubleComplex(__hiloint2double(b_real_hi[threadIdx.x][j + threadIdx.y],
                                                             b_real_lo[threadIdx.x][j + threadIdx.y]),
                                            __hiloint2double(b_imag_hi[threadIdx.x][j + threadIdx.y],
                                                             b_imag_lo[threadIdx.x][j + threadIdx.y]));
      ...
      if (m > 1) { caxpy(m - 1, x[0], &a_imag[0][1], &a_real[0][1], &x[1]); if (diag == CBlasNonUnit) x[1] = cuCdivf(x[1], make_cuComplex(a_real[1][1], a_imag[1][1]));
      if (m > 2) { caxpy(m - 2, x[1], &a_imag[1][2], &a_real[1][2], &x[2]); if (diag == CBlasNonUnit) x[2] = cuCdivf(x[2], make_cuComplex(a_real[2][2], a_imag[2][2]));
      if (m > 3) { caxpy(m - 3, x[2], &a_imag[2][3], &a_real[2][3], &x[3]); if (diag == CBlasNonUnit) x[3] = cuCdivf(x[3], make_cuComplex(a_real[3][3], a_imag[3][3])); }}}}
    ...
    if ((uplo == CBlasUpper && transA == CBlasNoTrans) ||
      ...
      while (n > 0) {
        ...
        while (k > 0) {
          ...
          #pragma unroll
          for (int l = 0; l < nb; l++)
            caxpy(_B[l * ldb], a_imag[l], a_real[l], x);
        ...
        if (n > 1) { caxpy(3, x[0], &a_imag[0][1], &a_real[0][1], &x[1]); if (diag == CBlasNonUnit) x[1] = cuCdivf(x[1], make_cuComplex(a_real[1][1], a_imag[1][1]));
        if (n > 2) { caxpy(2, x[1], &a_imag[1][2], &a_real[1][2], &x[2]); if (diag == CBlasNonUnit) x[2] = cuCdivf(x[2], make_cuComplex(a_real[2][2], a_imag[2][2]));
        if (n > 3) { caxpy(1, x[2], &a_imag[2][3], &a_real[2][3], &x[3]); if (diag == CBlasNonUnit) x[3] = cuCdivf(x[3], make_cuComplex(a_real[3][3], a_imag[3][3])); }}}}
      ...
      // Update X unrolled (forward loop)
      if (n > 0) { if (diag == CBlasNonUnit) x[0] = cuCdivf(x[0], make_cuComplex(a_real[0][0], a_imag[0][0]));
      if (n > 1) { caxpy(n - 1, x[0], &a_imag[0][1], &a_real[0][1], &x[1]); if (diag == CBlasNonUnit) x[1] = cuCdivf(x[1], make_cuComplex(a_real[1][1], a_imag[1][1]));
      if (n > 2) { caxpy(n - 2, x[1], &a_imag[1][2], &a_real[1][2], &x[2]); if (diag == CBlasNonUnit) x[2] = cuCdivf(x[2], make_cuComplex(a_real[2][2], a_imag[2][2]));
      if (n > 3) { caxpy(n - 3, x[2], &a_imag[2][3], &a_real[2][3], &x[3]); if (diag == CBlasNonUnit) x[3] = cuCdivf(x[3], make_cuComplex(a_real[3][3], a_imag[3][3])); }}}}
      ...
      if (nn > 0) {
        ...
        switch (nn - 1) {
          case 3: if (diag == CBlasNonUnit) x[3] = cuCdivf(x[3], make_cuComplex(a_real[3][3], a_imag[3][3])); caxpy(3, x[3], a_imag[3], a_real[3], x);
          case 2: if (diag == CBlasNonUnit) x[2] = cuCdivf(x[2], make_cuComplex(a_real[2][2], a_imag[2][2])); caxpy(2, x[2], a_imag[2], a_real[2], x);
          case 1: if (diag == CBlasNonUnit) x[1] = cuCdivf(x[1], make_cuComplex(a_real[1][1], a_imag[1][1])); caxpy(1, x[1], a_imag[1], a_real[1], x);
      ...
      while (j >= 0) {
        ...
        // Update X from right to left
        if (diag == CBlasNonUnit) x[3] = cuCmulf(x[3], make_cuComplex(a_real[3][3], a_imag[3][3])); caxpy(3, x[3], a_imag[3], a_real[3], x);
        if (diag == CBlasNonUnit) x[2] = cuCmulf(x[2], make_cuComplex(a_real[2][2], a_imag[2][2])); caxpy(2, x[2], a_imag[2], a_real[2], x);
        if (diag == CBlasNonUnit) x[1] = cuCmulf(x[1], make_cuComplex(a_real[1][1], a_imag[1][1])); caxpy(1, x[1], a_imag[1], a_real[1], x);
