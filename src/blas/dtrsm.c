...
void dtrsm(CBlasSide side, CBlasUplo uplo, CBlasTranspose transA, CBlasDiag diag, size_t m, size_t n, double alpha, const double * A, size_t lda, double * B, size_t ldb) {
...
static inline CUresult cuDtrsm2(CUmodule module, CBlasSide side, CBlasUplo uplo, CBlasTranspose trans, CBlasDiag diag, size_t m, size_t n, double alpha, CUdeviceptr A, size_t lda, CUdeviceptr B, size_t ldb, CUstream stream) {
  size_t nRowA = (side == CBlasLeft) ? m : n;
...
CUresult cuDtrsm(CUmodule module, CBlasSide side, CBlasUplo uplo, CBlasTranspose trans, CBlasDiag diag, size_t m, size_t n, double alpha, CUdeviceptr A, size_t lda, CUdeviceptr B, size_t ldb, CUstream stream) {
  return cuDtrsm2(module, side, uplo, trans, diag, m, n, alpha, A, lda, B, ldb, stream);
}

CUresult cuMultiGPUDtrsm(CUcontext * contexts, int deviceCount, CBlasSide side, CBlasUplo uplo, CBlasTranspose trans, CBlasDiag diag, size_t m, size_t n, double alpha, const double * restrict A, size_t lda, double * restrict B, size_t ldb) {
  return CUDA_SUCCESS;
}

#if 0
// gcc -I../../include -I/opt/cuda/include -march=native -O2 -pipe -std=c99 -pedantic -Wall -Wextra -Wconversion -c dtrsm.c
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <float.h>
...
static void dtrsm_ref(CBlasSide, CBlasUplo, CBlasTranspose, CBlasDiag, size_t, size_t, double, const double * restrict, size_t, double * restrict, size_t);
...
int main(int argc, char * argv[]) {
  ...
  if (side == CBlasLeft) {
    if ((A = malloc2D(m, m, &lda, sizeof(double))) == NULL) {
    ...
    dlda /= sizeof(double);
    CU_ERROR_CHECK(cuMemcpyHtoD2D(dA, dlda, 0, 0, A, lda, 0, 0, m, m, sizeof(double)));
#endif
  ...
  if ((B = malloc2D(m, n, &ldb, sizeof(double))) == NULL) {
    fprintf(stderr, "Unable to allocate B\n");
    return CUDA_ERROR_OUT_OF_MEMORY;
  }
  if ((refB = malloc2D(m, n, &ldb, sizeof(double))) == NULL) {
  ...
  for (size_t j = 0; j < n; j++)
    memcpy(&refB[j * ldb], &B[j * ldb], m * sizeof(double));

#ifdef GPU
  CU_ERROR_CHECK(cuMemAllocPitch(&dB, &dldb, m * sizeof(double), n, sizeof(double)));
  dldb /= sizeof(double);
  CU_ERROR_CHECK(cuMemcpyHtoD2D(dB, dldb, 0, 0, B, ldb, 0, 0, m, n, sizeof(double)));
#endif
...
#ifdef GPU
  CUmodule module;
  CU_ERROR_CHECK(cuModuleLoad(&module, "dtrsm.cubin"));

  CU_ERROR_CHECK(cuDtrsm(module, side, uplo, trans, diag, m, n, alpha, dA, dlda, dB, dldb, NULL));

  CU_ERROR_CHECK(cuMemcpyDtoH2D(B, ldb, 0, 0, dB, dldb, 0, 0, m, n, sizeof(double)));
#else
#ifdef MULTIGPU
  ...
  if (m <= 8 && n <= 8) {
    fprintf2D(stdout, "Reference DTRSM", m, n, refB, ldb);
    fprintf2D(stdout, "DTRSM", m, n, B, ldb);
  }
  ...
  for (size_t j = 0; j < n; j++) {
    for (size_t i = 0; i < m; i++) {
      ...
      if (passed) {
        ...
        if (diff > (double)flops * 2.0 * DBL_EPSILON)
          passed = false;
      }
    }
  }
...
void dtrsm_ref(CBlasSide side, CBlasUplo uplo, CBlasTranspose transA, CBlasDiag diag, size_t m, size_t n, double alpha, const double * A, size_t lda, double * B, size_t ldb) {
  size_t nRowA = (side == CBlasLeft) ? m : n;
...
static double gaussian(double mean, double variance) {
  ...
  double r = sqrt(-1.0 * log(u0));
  double phi = 2.0 * 3.1415926535897932384626433832795 * u1;
  next = r * sin(phi);
  hasNext = true;

  return r * cos(phi) * variance + mean;
}
