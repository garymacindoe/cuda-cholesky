...
static CUresult background_dgemm(const void * a) {
  ...
  if ((data = pthread_getspecific(key)) == NULL) {
    ...
    CU_ERROR_CHECK(cuMemAllocPitch(&data->C, &data->ldc, mb * sizeof(double), nb,
                                   sizeof(double)));
    data->ldc /= sizeof(double);

    if (transA == CBlasNoTrans) {
      // A is mb * kb
      CU_ERROR_CHECK(cuMemAllocPitch(&data->A0, &data->lda, mb * sizeof(double), kb,
                                     sizeof(double)));
      CU_ERROR_CHECK(cuMemAllocPitch(&data->A1, &data->lda, mb * sizeof(double), kb,
                                     sizeof(double)));
      data->lda /= sizeof(double);
    }
    else {
      // A is kb * mb
      CU_ERROR_CHECK(cuMemAllocPitch(&data->A0, &data->lda, kb * sizeof(double), mb,
                                     sizeof(double)));
      CU_ERROR_CHECK(cuMemAllocPitch(&data->A1, &data->lda, kb * sizeof(double), mb,
                                     sizeof(double)));
      data->lda /= sizeof(double);
    }
    ...
    if (transB == CBlasNoTrans) {
      // B is kb * nb
      CU_ERROR_CHECK(cuMemAllocPitch(&data->B0, &data->ldb, kb * sizeof(double), nb,
                                     sizeof(double)));
      CU_ERROR_CHECK(cuMemAllocPitch(&data->B1, &data->ldb, kb * sizeof(double), nb,
                                     sizeof(double)));
      data->ldb /= sizeof(double);
    }
    else {
      // B is nb * kb
      CU_ERROR_CHECK(cuMemAllocPitch(&data->B0, &data->ldb, nb * sizeof(double), kb,
                                    sizeof(double)));
      CU_ERROR_CHECK(cuMemAllocPitch(&data->B1, &data->ldb, nb * sizeof(double), kb,
                                    sizeof(double)));
      data->ldb /= sizeof(double);
    }
  }
  ...
  CU_ERROR_CHECK(cuMemcpyHtoD2DAsync(data->C, data->ldc, 0, 0, C, ldc, 0, 0,
                                     m, n, sizeof(double), data->compute));

  // Perform C *= beta on the compute stream to ensure C has finished copying
  CU_ERROR_CHECK(cuDgemm(data->module, CBlasNoTrans, CBlasNoTrans, m, n, 0,
  ...
  if (alpha != zero && k > 0) {
    ...
    if (transB == CBlasNoTrans) {
      if (transA == CBlasNoTrans) {
        ...
        const size_t lb = min(k, kb);
        CU_ERROR_CHECK(cuMemcpyHtoD2DAsync(data->A0, data->lda, 0, 0, A, lda, 0, 0,
                                           lb, m, sizeof(double), data->compute));
        CU_ERROR_CHECK(cuMemcpyHtoD2DAsync(data->B0, data->ldb, 0, 0, B, ldb, 0, 0,
                                           lb, n, sizeof(double), data->compute));
      ...
      if (transA == CBlasNoTrans) {
        ...
        CU_ERROR_CHECK(cuMemcpyHtoD2DAsync(data->A0, data->lda, 0, 0, A, lda, 0, 0,
                                           m, lb, sizeof(double), data->compute));
        CU_ERROR_CHECK(cuMemcpyHtoD2DAsync(data->B0, data->ldb, 0, 0, B, ldb, 0, 0,
                                           n, lb, sizeof(double), data->compute));
