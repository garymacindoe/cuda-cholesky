...
static CUresult background_dgemm(const void * a) {
  ...
  const size_t lda = plan->lda / sizeof(double);
  CUdeviceptr B0 = plan->B;
  CUdeviceptr B1 = plan->B + plan->ldb * ((args->transB == CBlasNoTrans) ? nb : kb);
  const size_t ldb = plan->ldb / sizeof(double);
  CUdeviceptr C = plan->C;
  const size_t ldc = plan->ldc / sizeof(double);
  ...
  CU_ERROR_CHECK(cuDgemm(plan->dgemm, CBlasNoTrans, CBlasNoTrans,
                         args->m, args->n, 0,
                         zero, 0, ldc, 0, 0,
    ...
    if (args->transB == CBlasNoTrans) {
      if (args->transA == CBlasNoTrans) {
        ...
        for (size_t l = 0; l < args->k; l += kb) {
          ...
          CU_ERROR_CHECK(cuDgemm(plan->dgemm, args->transA, args->transB,
