...
static CUresult background_dgemm(const void * a) {
  ...
  // Allocate C
  CU_ERROR_CHECK(cuMemAllocPitch(&C, &ldc, m * sizeof(double), n, sizeof(double)));
  ldc /= sizeof(double);
  ...
  // Perform C *= beta
  CU_ERROR_CHECK(cuDgemm(module, CBlasNoTrans, CBlasNoTrans,
                         m, n, 0,
  ...
  if (alpha != zero && k > 0) {
    ...
    if (transB == CBlasNoTrans) {
      // B is k * n
      CU_ERROR_CHECK(cuMemAllocPitch(&B0, &ldb, kb * sizeof(double), n, sizeof(double)));
      CU_ERROR_CHECK(cuMemAllocPitch(&B1, &ldb, kb * sizeof(double), n, sizeof(double)));
      ldb /= sizeof(double);

      if (transA == CBlasNoTrans) {
        // A is m * k
        CU_ERROR_CHECK(cuMemAllocPitch(&A0, &lda, m * sizeof(double), kb, sizeof(double)));
        CU_ERROR_CHECK(cuMemAllocPitch(&A1, &lda, m * sizeof(double), kb, sizeof(double)));
        lda /= sizeof(double);
        ...
        // A is k * m
        CU_ERROR_CHECK(cuMemAllocPitch(&A0, &lda, kb * sizeof(double), m, sizeof(double)));
        CU_ERROR_CHECK(cuMemAllocPitch(&A1, &lda, kb * sizeof(double), m, sizeof(double)));
        lda /= sizeof(double);
      ...
      // B is n * k
      CU_ERROR_CHECK(cuMemAllocPitch(&B0, &ldb, n * sizeof(double), kb, sizeof(double)));
      CU_ERROR_CHECK(cuMemAllocPitch(&B1, &ldb, n * sizeof(double), kb, sizeof(double)));
      ldb /= sizeof(double);
...
CUresult cuMultiGPUDgemm(CUmultiGPU multiGPU,
...
#pragma omp parallel for
  ...
  struct dgemm_args args = { transA, transB,
                             m, n, k,
                             alpha, A, lda, B, ldb,
                             beta, C, ldc };
    ...
    if (transA == CBlasNoTrans) {
      for (size_t j = 0; j < n; j += nb) {
        ...
        for (size_t i = 0; i < m; i += mb) {
          ...
          args.B = &B[j * ldb];
          args.C = &C[j * ldc + i];
          CU_ERROR_CHECK(cuTaskSchedule(&tasks[t++], multiGPU, background_dgemm,
                                        &args, sizeof(struct dgemm_args)));
        }
      }
    }
    else {
