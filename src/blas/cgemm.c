...
static CUresult init(const void * a) {
  struct cgemm_data * data = *(struct cgemm_data **)a;
  ...
  CU_ERROR_CHECK(cuMemAllocPitch(&data->C, &data->ldc,
                                 data->m * sizeof(float complex), data->n,
  ...
  if (data->transA == CBlasNoTrans) {
    ...
    CU_ERROR_CHECK(cuMemAllocPitch(&data->A0, &data->lda,
                                   data->m * sizeof(float complex), data->k,
                                   sizeof(float complex)));
    ...
    // A is k * m
    CU_ERROR_CHECK(cuMemAllocPitch(&data->A0, &data->lda,
                                   data->k * sizeof(float complex), data->m,
                                   sizeof(float complex)));
    CU_ERROR_CHECK(cuMemAllocPitch(&data->A1, &data->lda,
                                   data->k * sizeof(float complex), data->m,
                                   sizeof(float complex)));
  ...
  if (data->transB == CBlasNoTrans) {
    ...
    CU_ERROR_CHECK(cuMemAllocPitch(&data->B0, &data->ldb,
                                   data->k * sizeof(float complex), data->n,
                                   sizeof(float complex)));
    ...
    // B is n * k
    CU_ERROR_CHECK(cuMemAllocPitch(&data->B0, &data->ldb,
                                   data->n * sizeof(float complex), data->k,
                                   sizeof(float complex)));
    CU_ERROR_CHECK(cuMemAllocPitch(&data->B1, &data->ldb,
                                   data->n * sizeof(float complex), data->k,
                                   sizeof(float complex)));
...
static CUresult background_cgemm(const void * a) {
  ...
  const struct cgemm_data * data = args->data;
  ...
  float complex alpha = args->alpha, beta = args->beta;
  const float complex * A = args->A, * B = args->B;
  ...
  CU_ERROR_CHECK(cuCgemm(module, CBlasNoTrans, CBlasNoTrans, m, n, 0,
...
#pragma omp parallel for
  ...
  const size_t kb = (transA == CBlasNoTrans) ?  128 : 128;
  ...
  struct cgemm_data * data[nThreads];
  for (int i = 0; i < nThreads; i++) {
    if ((data[i] = malloc(sizeof(struct cgemm_data))) == NULL) {
    ...
    CU_ERROR_CHECK(cuTaskCreate(&task, init, &data[i], sizeof(struct cgemm_data *)));
  ...
  for (int i = 0; i < nThreads; i++) {
    CU_ERROR_CHECK(cuTaskCreate(&task, cleanup, &data[i], sizeof(struct cgemm_data *)));
    CU_ERROR_CHECK(cuTaskQueuePush(queue, task));
    CU_ERROR_CHECK(cuThreadRunTask(threads[i], task));
  }
