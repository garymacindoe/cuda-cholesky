...
// #ifdef MKL_ILP64
//   extern void ssyrk_(const char *, const char *, const long *, const long *, const float *, const float *, const long *, const float *, float *, const long *);
// #else
//   extern void ssyrk_(const char *, const char *, const int *, const int *, const float *, const float *, const int *, const float *, float *, const int *);
// #endif
void ssyrk(CBlasUplo uplo, CBlasTranspose trans, size_t n, size_t k, float alpha, const float * restrict A, size_t lda, float beta, float * restrict C, size_t ldc) {
  ...
  int info = 0;
// #ifdef MKL_ILP64
//   ssyrk_((const char *)&uplo, (const char *)&trans, (const long *)&n, (const long *)&k, &alpha, A, (const long *)&lda, &beta, C, (const long *)&ldc);
// #else
//   ssyrk_((const char *)&uplo, (const char *)&trans, (const int *)&n, (const int *)&k, &alpha, A, (const int *)&lda, &beta, C, (const int *)&ldc);
// #endif
//   return;
  if (lda < max(1, nRowA))
    info = 7;
...
static inline CUresult cuSsyrk2(CUmodule module, CBlasUplo uplo, CBlasTranspose trans, size_t n, size_t k, float alpha, CUdeviceptr A, size_t lda, float beta, CUdeviceptr C, size_t ldc, CUstream stream) {
  size_t nRowA = (trans == CBlasNoTrans) ? n : k;
...
CUresult cuSsyrk(CUmodule module, CBlasUplo uplo, CBlasTranspose trans, size_t n, size_t k, float alpha, CUdeviceptr A, size_t lda, float beta, CUdeviceptr C, size_t ldc, CUstream compute) {
  size_t nRowA = (trans == CBlasNoTrans) ? n : k;
  ...
  if (trans == CBlasNoTrans) {
    if (uplo == CBlasLower) {
      ...
      if (n < nb && k < kb)
        return cuSsyrk2(module, uplo, trans, n, k, alpha, A, lda, beta, C, ldc, compute);
      ...
      CU_ERROR_CHECK(cuMemAllocHost((void **)&hA0, (hlda0 = (nb + 3u) & ~3u) * kb * sizeof(float)));
      CU_ERROR_CHECK(cuMemAllocHost((void **)&hA1, (hlda1 = (nb + 3u) & ~3u) * kb * sizeof(float)));
      CU_ERROR_CHECK(cuMemAllocHost((void **)&hC,  (hldc  = (nb + 3u) & ~3u) * nb * sizeof(float)));

      for (size_t j = 0; j < n; j += nb) {
        const size_t jb = min(nb, n - j);

        if (j + jb < n)
          CU_ERROR_CHECK(cuSgemm(sgemm, trans, CBlasTrans, n - j - jb, jb, k, alpha, A + (j + jb) * sizeof(float), lda, A + j * sizeof(float), lda, beta, C + (j * ldc + j + jb) * sizeof(float), ldc, compute));

        CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hC,  hldc,  0, 0, C, ldc, j, j, jb, jb, sizeof(float), copy1));
        CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hA0, hlda0, 0, 0, A, lda, j, 0, jb, min(kb, k), sizeof(float), copy0));

        CU_ERROR_CHECK(cuStreamSynchronize(copy1));
        ssyrk(uplo, trans, jb, 0, zero, NULL, lda, beta, hC, hldc);

        for (size_t l = 0; l < k; l += kb) {
          if (l + kb < k)
            CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hA1, hlda1, 0, 0, A, lda, j, l + kb, jb, min(kb, k - l - kb), sizeof(float), copy1));

          CU_ERROR_CHECK(cuStreamSynchronize(copy0));
          ssyrk(uplo, trans, jb, min(kb, k - l), alpha, hA0, hlda0, one, hC, hldc);

          l += kb;
          if (l < k) {
            if (l + kb < k)
              CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hA0, hlda0, 0, 0, A, lda, j, l + kb, jb, min(kb, k - l - kb), sizeof(float), copy0));

            CU_ERROR_CHECK(cuStreamSynchronize(copy1));
            ssyrk(uplo, trans, jb, min(kb, k - l), alpha, hA1, hlda1, one, hC, hldc);
          }
        }

        CU_ERROR_CHECK(cuMemcpyHtoD2DAsync(C, ldc, j, j, hC, hldc, 0, 0, jb, jb, sizeof(float), copy0));
      }
    }
    else {
      const size_t nb = 64;
      ...
      for (size_t j = 0; j < n; j += nb) {
        ...
        if (j + jb < n)
          CU_ERROR_CHECK(cuSgemm(sgemm, trans, CBlasTrans, jb, n - j - jb, k, alpha, A + j * sizeof(float), lda, A + (j + jb) * sizeof(float), lda, beta, C + ((j + jb) * ldc + j) * sizeof(float), ldc, compute));
    ...
    if (uplo == CBlasLower) {
      ...
      CU_ERROR_CHECK(cuMemAllocHost((void **)&hA0, (hlda0 = (kb + 3u) & ~3u) * nb * sizeof(float)));
      CU_ERROR_CHECK(cuMemAllocHost((void **)&hA1, (hlda1 = (kb + 3u) & ~3u) * kb * sizeof(float)));
      CU_ERROR_CHECK(cuMemAllocHost((void **)&hC,  (hldc  = (nb + 3u) & ~3u) * nb * sizeof(float)));
      ...
      for (size_t j = 0; j < n; j += nb) {
        ...
        if (j + jb < n)
          CU_ERROR_CHECK(cuSgemm(sgemm, trans, CBlasNoTrans, n - j - jb, jb, k, alpha, A + (j + jb) * lda * sizeof(float), lda, A + j * lda * sizeof(float), lda, beta, C + (j * ldc + j + jb) * sizeof(float), ldc, compute));

        CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hC,  hldc,  0, 0, C, ldc, j, j, jb, jb, sizeof(float), copy1));
        CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hA0, hlda0, 0, 0, A, lda, 0, j, min(kb, k), jb, sizeof(float), copy0));
        ...
        for (size_t l = 0; l < k; l += kb) {
          if (l + kb < k)
            CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hA1, hlda1, 0, 0, A, lda, l + kb, j, min(kb, k - l - kb), jb, sizeof(float), copy1));
          ...
          if (l < k) {
            if (l + kb < k)
              CU_ERROR_CHECK(cuMemcpyDtoH2DAsync(hA0, hlda0, 0, 0, A, lda, l + kb, j, min(kb, k - l - kb), jb, sizeof(float), copy0));
      ...
      for (size_t j = 0; j < n; j += nb) {
        ...
        if (j + jb < n)
          CU_ERROR_CHECK(cuSgemm(sgemm, CBlasTrans, CBlasNoTrans, jb, n - j - jb, k, alpha, A + j * lda * sizeof(float), lda, A + (j + jb) * lda * sizeof(float), lda, beta, C + ((j + jb) * ldc + j) * sizeof(float), ldc, compute));
...
#if 0
// gcc -I../../include -I/opt/cuda/include -march=native -O2 -pipe -std=c99 -pedantic -Wall -Wextra -Wconversion -c ssyrk.c
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <float.h>
...
static void ssyrk_ref(CBlasUplo, CBlasTranspose, size_t, size_t, float, const float * restrict, size_t, float, float * restrict, size_t);
...
int main(int argc, char * argv[]) {
  ...
  if (trans == CBlasNoTrans) {
    if ((A = malloc2D(n, k, &lda, sizeof(float))) == NULL) {
    ...
    dlda /= sizeof(float);
    CU_ERROR_CHECK(cuMemcpyHtoD2D(dA, dlda, 0, 0, A, lda, 0, 0, n, k, sizeof(float)));
#endif
    ...
    if ((A = malloc2D(k, n, &lda, sizeof(float))) == NULL) {
      fprintf(stderr, "Unable to allocate A\n");
      return CUDA_ERROR_OUT_OF_MEMORY;
    }
    ...
    dlda /= sizeof(float);
    CU_ERROR_CHECK(cuMemcpyHtoD2D(dA, dlda, 0, 0, A, lda, 0, 0, k, n, sizeof(float)));
#endif
  ...
  if ((C = malloc2D(n, n, &ldc, sizeof(float))) == NULL) {
    fprintf(stderr, "Unable to allocate C\n");
    return CUDA_ERROR_OUT_OF_MEMORY;
  }
  if ((refC = malloc2D(n, n, &ldc, sizeof(float))) == NULL) {
  ...
  for (size_t j = 0; j < n; j++)
    memcpy(&refC[j * ldc], &C[j * ldc], n * sizeof(float));

#ifdef GPU
  CU_ERROR_CHECK(cuMemAllocPitch(&dC, &dldc, n * sizeof(float), n, sizeof(float)));
  dldc /= sizeof(float);
  CU_ERROR_CHECK(cuMemcpyHtoD2D(dC, dldc, 0, 0, C, ldc, 0, 0, n, n, sizeof(float)));
#endif
  ...
  CU_ERROR_CHECK(cuMemcpyDtoH2D(C, ldc, 0, 0, dC, dldc, 0, 0, n, n, sizeof(float)));
#else
#ifdef MULTIGPU
  ...
  if (n <= 8 && k <= 8) {
    fprintf2D(stdout, "Reference SSYRK", n, n, refC, ldc);
    fprintf2D(stdout, "SSYRK", n, n, C, ldc);
  }
...
static void ssyrk_ref(CBlasUplo uplo, CBlasTranspose trans, size_t n, size_t k, float alpha, const float * restrict A, size_t lda, float beta, float * restrict C, size_t ldc) {
