...
// y(1:n) += alpha * x(1:n)
__device__ void caxpy(int n, cuComplex a, float * b_real, float * b_imag, cuComplex * c) {
  c[0] = cuCfmaf(a, make_cuComplex(b_real[0], b_imag[0]), c[0]); if (1 >= n) return;
  c[1] = cuCfmaf(a, make_cuComplex(b_real[1], b_imag[1]), c[1]); if (2 >= n) return;
  c[2] = cuCfmaf(a, make_cuComplex(b_real[2], b_imag[2]), c[2]); if (3 >= n) return;
  c[3] = cuCfmaf(a, make_cuComplex(b_real[3], b_imag[3]), c[3]);
}
