...
static void zlauum(CBlasUplo uplo,
                   size_t n,
                   double complex * restrict A, size_t lda,
                   long * restrict info) {
  *info = 0;
  ...
  *info = 0;
  ...
  if (uplo == CBlasUpper) {
    ...
    for (size_t j = 0; j < n; j += nb) {
      ...
      ztrtri(CBlasUpper, CBlasNonUnit, jb, B, ldb, info);
...
//       CU_ERROR_CHECK(cuStreamSynchronize(stream1));
    ...
    do {
      ...
      CU_ERROR_CHECK(cuStreamSynchronize(stream1));
      /* Form the inverse of the diagonal block using the CPU */
      ztrtri(CBlasLower, CBlasNonUnit, jb, B, ldb, info);
      /* Check for singular matrix */
      if (*info != 0) {
        *info += (long)j;
        break;
...
CUresult cuMultiGPUZpotri(CUmultiGPUBlasHandle handle,
  ...
  *info = 0;
  ...
  if (uplo == CBlasUpper) {
    ...
    for (size_t j = 0; j < n; j += nb) {
      const size_t jb = min(nb, n - j);
      CU_ERROR_CHECK(cuMultiGPUZtrmm(handle, CBlasLeft, CBlasUpper, CBlasNoTrans, CBlasNonUnit,
                                     j, jb, one, A, lda, &A[j * lda], lda));
      CU_ERROR_CHECK(cuMultiGPUZtrsm(handle, CBlasRight, CBlasUpper, CBlasNoTrans, CBlasNonUnit,
                                     j, jb, -one, &A[j * lda + j], lda, &A[j * lda], lda));
      ztrtri(CBlasUpper, CBlasNonUnit, jb, &A[j * lda + j], lda, info);
      if (*info != 0) {
        *info += (long)j;
        return CUDA_ERROR_INVALID_VALUE;
      }
    ...
    do {
      ...
      const size_t jb = min(nb, n - j);
      if (j + jb < n) {
        CU_ERROR_CHECK(cuMultiGPUZtrmm(handle, CBlasLeft, CBlasLower, CBlasNoTrans, CBlasNonUnit,
                                       n - j - jb, jb,
                                       one, &A[(j + jb) * lda + j + jb], lda,
                                       &A[j * lda + j + jb], lda));
        CU_ERROR_CHECK(cuMultiGPUZtrsm(handle, CBlasRight, CBlasLower, CBlasNoTrans, CBlasNonUnit,
                                       n - j - jb, jb,
                                       -one, &A[j * lda + j], lda,
                                       &A[j * lda + j + jb], lda));
      }
      ztrtri(CBlasLower, CBlasNonUnit, jb, &A[j * lda + j], lda, info);
      if (*info != 0) {
        *info += (long)j;
        return CUDA_ERROR_INVALID_VALUE;
      }
