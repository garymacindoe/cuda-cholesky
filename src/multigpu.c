 ...
 */
static CUresult cuThreadCreate(CUthread * thread) {
 ...
 */
static CUresult cuThreadDestroy(CUthread thread) {
  // Lock the queue
 ...
 *         task to the thread.
 */
static CUresult cuThreadRunTask(CUthread thread, CUtask task) {
...
CUresult cuMultiGPUSynchronize(CUmultiGPU mGPU) {
  ...
  for (int i = 0; i < mGPU->n; i++) {
    ...
    CU_ERROR_CHECK(cuMultiGPURunTask(mGPU, task));
    CU_ERROR_CHECK(cuTaskDestroy(task, &result));
    if (result != CUDA_SUCCESS)
      return result;
  }
