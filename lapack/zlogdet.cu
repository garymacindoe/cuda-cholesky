...
__global__ void reduce(const cuDoubleComplex * x, double * temp, int incx, int n) {
  ...
  for (int i = blockIdx.x * (bs * 2) + threadIdx.x; i < n; i += gs) {
    sum += logf(cuCreal(x[i * incx]));    // log x when reading from global memory

    // ensure we don't read out of bounds -- this is optimized away for powerOf2 sized arrays
    if (nIsPow2 || i + bs < n)
      sum += logf(cuCreal(x[(i + bs) * incx]));   // log x when reading from global memory
  }
  ...
  if (gridDim.x > 1) {
    ...
    if (amLast) {
      ...
      if (threadIdx.x == 0) {
        temp[0] = 2.0 * shared[0];     // calculate 2 * sum(log(x))
...
template __global__ void reduce<512,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<256,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<128,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce< 64,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce< 32,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce< 16,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  8,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  4,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  2,  true>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  1,  true>(const cuDoubleComplex *, double *, int, int);

template __global__ void reduce<512, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<256, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<128, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce< 64, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce< 32, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce< 16, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  8, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  4, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  2, false>(const cuDoubleComplex *, double *, int, int);
template __global__ void reduce<  1, false>(const cuDoubleComplex *, double *, int, int);
