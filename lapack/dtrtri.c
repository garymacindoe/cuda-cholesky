  ...
  *info = 0;
  ...
  if (uplo == CBlasUpper) {
    for (size_t j = 0; j < n; j += nb) {
      ...
      CU_ERROR_CHECK(cuDtrmm(handle->blas_handle,
                             CBlasLeft, CBlasUpper, CBlasNoTrans, diag, j, jb,
                             one, A, lda, A + j * lda * sizeof(double), lda, stream0));

      /* Then update the column again using the small square matrix on the
       * diagonal below */
      CU_ERROR_CHECK(cuDtrsm(handle->blas_handle,
                             CBlasRight, CBlasUpper, CBlasNoTrans, diag, j, jb,
                             -one, A + (j * lda + j) * sizeof(double), lda,
