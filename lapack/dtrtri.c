  ...
  if (uplo == CBlasUpper) {
    ...
    for (size_t j = 0, nb = 1; j < n; j += nb, nb = min(n / 2, nb * 2)) {
      ...
      if (bcc_dtoh) {
        ...
        CU_ERROR_CHECK(cuMemcpyDtoHAsync(D, A + j * lda * sizeof(double), n * jb * sizeof(double), stream1));
      ...
      if (bcc_htod)
        CU_ERROR_CHECK(cuMemcpyHtoDAsync(A + j * lda * sizeof(double), D, n * jb * sizeof(double), stream1));
      else
