...
__global__ void reduce(const cuComplex * x, float * temp, int incx, int n) {
  ...
  for (int i = blockIdx.x * (bs * 2) + threadIdx.x; i < n; i += gs) {
    sum += logf(cuCrealf(x[i * incx]));    // log x when reading from global memory

    // ensure we don't read out of bounds -- this is optimized away for powerOf2 sized arrays
    if (nIsPow2 || i + bs < n)
      sum += logf(cuCrealf(x[(i + bs) * incx]));   // log x when reading from global memory
  }
...
template __global__ void reduce<512,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce<256,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce<128,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce< 64,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce< 32,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce< 16,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce<  8,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce<  4,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce<  2,  true>(const cuComplex *, float *, int, int);
template __global__ void reduce<  1,  true>(const cuComplex *, float *, int, int);

template __global__ void reduce<512, false>(const cuComplex *, float *, int, int);
template __global__ void reduce<256, false>(const cuComplex *, float *, int, int);
template __global__ void reduce<128, false>(const cuComplex *, float *, int, int);
template __global__ void reduce< 64, false>(const cuComplex *, float *, int, int);
template __global__ void reduce< 32, false>(const cuComplex *, float *, int, int);
template __global__ void reduce< 16, false>(const cuComplex *, float *, int, int);
template __global__ void reduce<  8, false>(const cuComplex *, float *, int, int);
template __global__ void reduce<  4, false>(const cuComplex *, float *, int, int);
template __global__ void reduce<  2, false>(const cuComplex *, float *, int, int);
template __global__ void reduce<  1, false>(const cuComplex *, float *, int, int);
