...
int main(int argc, char * argv[]) {
  ...
  CU_ERROR_CHECK(cuMemAllocPitch(&dX, &dldb, m * sizeof(float), n, sizeof(float)));
  dldb /= sizeof(float);
  ...
  CU_ERROR_CHECK(cuStrmm(module, side, uplo, trans, diag, m, n, alpha, dA, dlda, dB, dldb, dX, NULL));

  copy = (CUDA_MEMCPY2D){ 0, 0, CU_MEMORYTYPE_DEVICE, NULL, dX, NULL, dldb * sizeof(float),
