...
int main(int argc, char * argv[]) {
  ...
  if ((x = malloc(incx * n * sizeof(double complex))) == NULL) {
    fputs("Unable to allocate x\n", stderr);
    return -1;
  }
  CU_ERROR_CHECK(cuMemAlloc(&dx, incx * n * sizeof(double complex)));

  for (size_t j = 0; j < n; j++)
    x[j * incx] = (((double)rand() + 1.0) / (double)RAND_MAX) + 0.0 * I;

  CU_ERROR_CHECK(cuMemcpyHtoD(dx, x, incx * n * sizeof(double complex)));

  double res;
  CU_ERROR_CHECK(cuZlogdet(handle, dx, incx, n, &res, NULL));

  double sum = 0.0;
  double c = 0.0;
  for (size_t j = 0; j < n; j++) {
    double y = log(creal(x[j * incx])) - c;
    double t = sum + y;
    c = (t - sum) - y;
    sum = t;
  }
  ...
  const size_t bandwidth = n * sizeof(double complex);
