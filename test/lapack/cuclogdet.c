...
int main(int argc, char * argv[]) {
  ...
  CU_ERROR_CHECK(cuMemAlloc(&dx, incx * n * sizeof(float complex)));

  for (size_t j = 0; j < n; j++)
    x[j * incx] = (((float)rand() + 1.0f) / (float)RAND_MAX) + 0.0f * I;

  CU_ERROR_CHECK(cuMemcpyHtoD(dx, x, incx * n * sizeof(float complex)));

  float res;
  CU_ERROR_CHECK(cuClogdet(handle, dx, incx, n, &res, NULL));
