...
int main(int argc, char * argv[]) {
  ...
  ldx = (m + 1u) & ~1u;
  if ((X = malloc(ldx * n * sizeof(double))) == NULL) {
  ...
  dtrmm2(side, uplo, trans, diag, m, n, alpha, A, lda, B, ldb, X, ldx);

  bool passed = true;
  double diff = 0.0;
  for (size_t j = 0; j < n; j++) {
    for (size_t i = 0; i < m; i++) {
      double d = fabs(X[j * ldx + i] - refB[j * ldb + i]);
      if (d > diff)
        diff = d;
