...
int main(int argc, char * argv[]) {
  ...
  double complex alpha, * A, * B, * X, * refB;
  size_t lda, ldb, ldx;
  ...
  ldx = m;
  if ((X = malloc(ldx * n * sizeof(double complex))) == NULL) {
  ...
  ztrmm2(side, uplo, trans, diag, m, n, alpha, A, lda, B, ldb, X, ldx);

  bool passed = true;
  double rdiff = 0.0, idiff = 0.0;
  for (size_t j = 0; j < n; j++) {
    for (size_t i = 0; i < m; i++) {
      double d = fabs(creal(X[j * ldx + i]) - creal(refB[j * ldb + i]));
      if (d > rdiff)
        rdiff = d;

      double c = fabs(cimag(X[j * ldx + i]) - cimag(refB[j * ldb + i]));
      if (c > idiff)
        idiff = c;
