...
int main(int argc, char * argv[]) {
  ...
  float complex alpha, * A, * B, * X, * refB;
  size_t lda, ldb, ldx;
  ...
  ldx = (m + 1u) & ~1u;
  if ((X = malloc(ldx * n * sizeof(float complex))) == NULL) {
  ...
  ctrmm2(side, uplo, trans, diag, m, n, alpha, A, lda, B, ldb, X, ldx);

  bool passed = true;
  float rdiff = 0.0f, idiff = 0.0f;
  for (size_t j = 0; j < n; j++) {
    for (size_t i = 0; i < m; i++) {
      float d = fabsf(crealf(X[j * ldx + i]) - crealf(refB[j * ldb + i]));
      if (d > rdiff)
        rdiff = d;

      float c = fabsf(cimagf(X[j * ldx + i]) - cimagf(refB[j * ldb + i]));
      if (c > idiff)
        idiff = c;
