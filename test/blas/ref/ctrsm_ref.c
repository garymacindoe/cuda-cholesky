static void ctrsm_ref(CBlasSide side, CBlasUplo uplo, CBlasTranspose trans, CBlasDiag diag,
                      size_t m, size_t n,
    ...
    if (trans == CBlasNoTrans) {
      if (uplo == CBlasUpper) {
        for (size_t j = 0; j < n; j++) {
          size_t i = m - 1;
          do {
            float complex temp = alpha * B[j * ldb + i]; E[j * ldb + i] = F[j * ldb + i] = 3;
        ...
        for (size_t j = 0; j < n; j++) {
          for (size_t i = 0; i < m; i++) {
            ...
            if (trans == CBlasConjTrans) {
              for (size_t k = 0; k < i; k++) {
                temp -= conjf(A[i * lda + k]) * B[j * ldb + k]; E[j * ldb + i] += E[j * ldb + k] + 4; F[j * ldb + i] += F[j * ldb + k] + 4; }
              if (diag == CBlasNonUnit) { temp /= conjf(A[i * lda + i]); E[j * ldb + i] += 12; F[j * ldb + i] += 12; }
            }
            else {
              for (size_t k = 0; k < i; k++) {
      ...
      if (uplo == CBlasUpper) {
        ...
        do {
          for (size_t i = 0; i < m; i++) {
            ...
            if (trans == CBlasConjTrans) {
              for (size_t k = j + 1; k < n; k++) {
                temp -= conjf(A[k * lda + j]) * B[k * ldb + i]; E[j * ldb + i] += E[j * ldb + k] + 4; F[j * ldb + i] += F[j * ldb + k] + 4; }
              if (diag == CBlasNonUnit) { temp /= conjf(A[j * lda + j]); E[j * ldb + i] += 12; F[j * ldb + i] += 12; }
            }
            else {
              for (size_t k = j + 1; k < n; k++) {
