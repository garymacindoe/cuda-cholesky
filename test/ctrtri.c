...
int main(int argc, char * argv[]) {
  ...
  for (size_t j = 0; j < n; j++) {
    for (size_t i = 0; i < n; i++) {
      float r = fabsf(crealf(A[j * lda + i]) - crealf(refA[j * lda + i]));
      if (r > rdiff)
        rdiff = r;
...
static void ctrtri_ref(CBlasUplo uplo, CBlasDiag diag, size_t n, float complex * restrict A, size_t lda, long * restrict info) {
  *info = 0;
  if (lda < n)
    *info = -5;
  if (*info != 0) {
