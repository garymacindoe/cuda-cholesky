...
// TODO: proper error/performance analysis for potf2
#ifndef POTF2_ERROR
#define POTF2_ERROR
static inline size_t potf2_flops(size_t n) {
  const size_t mul = 1, add = 1;
  return n * n * mul * add;
}

static inline long double potf2_error(size_t i, CBlasUplo uplo, size_t n, long double epsilon) {
  const size_t mul = 1, add = 1;
  if (uplo == CBlasLower)
    i = n - i;
  return (i * mul + (i - 1) * add) * epsilon;
}
static inline size_t potf2_flops_complex(size_t n) {
  const size_t mul = 1, add = 1;
  return n * n * mul * add;
}

static inline long double potf2_error_real(size_t i, CBlasUplo uplo, size_t n, long double epsilon) {
  const size_t mul = 1, add = 1;
  if (uplo == CBlasLower)
    i = n - i;
  return (i * mul + (i - 1) * add) * epsilon;
...
static inline long double potf2_error_imag(size_t i, CBlasUplo uplo, size_t n, long double epsilon) {
  const size_t mul = 1, add = 1;
  if (uplo == CBlasLower)
    i = n - i;
  return (i * mul + (i - 1) * add) * epsilon;
...
#ifdef COMPLEX
#define REAL_ERROR(i, uplo) potf2_error_real(i, uplo, n, EPSILON)
#define IMAG_ERROR(i, uplo) potf2_error_imag(i, uplo, n, EPSILON)
#define FLOPS potf2_flops_complex(n)
#define EXTENDED_BASE long double
#define EXTENDED long double complex
#else
#define ERROR(i, uplo) potf2_error(i, uplo, n, EPSILON)
#define FLOPS potf2_flops(n)
#define EXTENDED long double
#endif

static void LINALG_FUNCTION2(gold, potf2)(CBlasUplo uplo, TYPE(matrix) * A, long * info) {
  *info = 0;
  if (A->m != A->n) {
    *info = -2;
    return;
...
static void TEST_FUNCTION(potf2)() {
#define F LINALG_FUNCTION(potf2)
#define PREFIX ""
#include "testpotf2.c"
#undef PREFIX
#undef F
...
static void TEST_FUNCTION2(lapack, potf2)() {
#define F LINALG_FUNCTION2(lapack, potf2)
...
static void TEST_FUNCTION2(cu, potf2)() {
#define F LINALG_FUNCTION2(cu, potf2)
#define PREFIX "cu"
#include "testpotf2.c"
#undef PREFIX
#undef F
...
#undef __SELF_INCLUDE
...
#endif
  LINALG_FUNCTION2(gold, potf2)(CBlasUpper, &B0.m, &infoB);
  ...
  if (outfile != NULL) {
    ...
    fprintf(outfile, PREFIX STRING(BLAS_PREC) "potf2U,%.3e,,%.3e\n", time, (float)FLOPS / time);
#else
...
#endif
  LINALG_FUNCTION2(gold, potf2)(CBlasLower, &B0.m, &infoB);
  ...
  if (outfile != NULL) {
    ...
    fprintf(outfile, PREFIX STRING(BLAS_PREC) "potf2L,%.3e,,%.3e\n", time, (float)FLOPS / time);
#else
    struct timeval start, stop;
    ERROR_CHECK_VOID(gettimeofday(&start, NULL), (strerror_t)strerror);
    for (size_t i = 0; i < iterations; i++)
